...
(function (window, undefined) {
    ...
    gecko.ajax = function (conf) {
		...
		var jsonp = conf.jsonp;//回调函数等号左边变量名
		var jsonpCallback = conf.jsonpCallback;//回调函数等号右边的变量值(与服务器返回的函数名要一致)
        ...
        data = data ? data : "";//默认 空
        dataType = dataType ? dataType : "text";//默认 text,还可以选择jsonp
		jsonp = jsonp ? jsonp : "callback";
		jsonpCallback = jsonpCallback ? jsonpCallback : 'ran_'+Math.random().toFixed(5)*100000;//默认为随机函数名，只有php接受变量名$_GET['callback']
		if(dataType=='text'){  //当dataType未定义时，按非jsonp方式调取数据
			var xhr = new XMLHttpRequest();//创建 XMLHttpRequest 对象
			if (type === "POST" || type === "post") {
				//加随机函数是防止浏览器缓存:浏览器为了提高用户访问同一页面的速度，会对页面数据进行缓存，当输入的参数没有变化的，浏览器认为是相同页面，直接调用缓存的信息。
                xhr.open("POST", url +((url.indexOf("?")==-1)?'?':'&')+ "ran=" +Math.random(), async);
				xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
				xhr.send(convertData(data));
			} else {
                xhr.open("GET", url +((url.indexOf("?")==-1)?'?':'&')+ "ran=" +Math.random(), async);
				xhr.send(null);
			}
            if(async){  //当为异步时
                xhr.onload = function () {  //这里选择不用xhr.onreadystatechange，因为onreadystatechange每次状态改变都会触发事件。
                    response()
                }
            }else { //当为同步时，不用事件监听函数
                response()
            }
            function response(){  //定义服务器响应后的事件
                if (xhr.readyState == 4) {
                    if (xhr.status == 200) {
                        success && success(xhr.responseText);
                    } else {
                        console.log('服务器有响应，但状态有问题！');
                    }
                } else {
                    console.log('服务器未响应')&&error();
                }
            }
        ...
        if(dataType=='jsonp'||dataType=='JSONP'){  //当dataType为jsonp方式调取数据
			eval(jsonpCallback+'=function(res){success(res)}') //先定义回调函数
			var script = document.createElement("script");
			script.src = url+((url.indexOf("?")==-1)?'?':'&')+jsonp+'='+jsonpCallback;
			document.body.appendChild(script);  //加到body最后，加载php传过来的JS回调函数并运行 
		}
    }
    //----------------------------ajax end----------------------------//
